...
function countCategory(categoryArray) {
  ...
  var remain = 5 - length;
  var id = "#count_category";

  if (remain > 0) {
    var text = "残り" + remain + "個入力可能";
    changeText(id, text);
  } else if (remain === 0) {
    var _text = "入力できる最大数です。";
    changeText(id, _text);
  } else {
    var isNormal = false;
    var _text2 = "最大数を超えています!";
    changeText(id, _text2);
...
function checkElement(array) {
  ...
  for (var i = 0; i < array.length; i++) {
    if (array[i] === '' || array[i].length === 0) {
      ...
      var isNormal = false;
      var text = "カテゴリが30文字を超えています!";
      changeText(id, text);
      changeClass(id, isNormal);
      return;
    } else {
      var _isNormal = true;
      changeClass(id, _isNormal);
    }
  }
...
function pushTag(array) {
  var tags = []; //カテゴリの要素数が5より上の場合は処理を中断
  ...
  for (var i = 0; i < array.length; i++) {
    var element = createElement("i", "fas fa-tape tag is-primary mr-1 mb-1", array[i]);
    tags.push(element);
  }
...
function countText(id, limit, length) {
  ...
  if (remain > 0) {
    var isNormal = true;
    var text = "残り" + remain + "文字入力可能";
    changeText(id, text);
    changeClass(id, isNormal);
  } else if (remain === 0) {
    var _isNormal2 = true;
    var _text3 = "入力できる最大文字数です。";
    changeText(id, _text3);
    changeClass(id, _isNormal2);
  } else {
    var _isNormal3 = false;
    var _text4 = "最大文字数を超えています！";
    changeText(id, _text4);
    changeClass(id, _isNormal3);
  }
} //ここまで
//共通の関数
...
function changeClass(id, isNormal) {
  var normalStatus = "has-text-primary";
  var abnormalStatus = "has-text-danger";
